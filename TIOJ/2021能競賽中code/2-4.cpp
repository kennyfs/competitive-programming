...
signed main(){
    ...
    for(int j=1;j<=n;++j){
        //0 0
        mi[0]=mi[1]=1e9;
        for(int i=0;i<k;++i)mi[0]=min(mi[0],fdp[j-1][0]+(s[i][j]=='1'));
        //1 0
        for(int i=0;i<k;++i)mi[1]=min(mi[1],fdp[j-1][1]+(s[i][j]=='1'));
        if(mi[0]>mi[1]){
        ...
        //0 1
        mi[0]=mi[1]=1e9;
        for(int i=0;i<k;++i)mi[0]=min(mi[0],fdp[j-1][0]+(s[i][j]=='0'));
        //1 1
        for(int i=0;i<k;++i)mi[1]=min(mi[1],fdp[j-1][1]+(s[i][j]=='0'));
        if(mi[0]>mi[1]){
